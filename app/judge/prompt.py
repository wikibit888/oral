...
def build_judge_prompt(
    ...
    *,
    ...
    # 称谓随模式（用户反馈 2026-06-07：情景报告诊断文本出现「考生」）：共享段
    # 以「考生」书写（雅思语境）；情景整体替换为「用户」——prompt 用词直接带动
    # judge 输出用词，非考试语境不该有考生/考官。
    subject = "考生" if mode == "ielts" else "用户"
    role_line = (
        "你是严格、专业的英语口语考官 / 诊断教练。一次性产出整份结构化报告。"
        if mode == "ielts"
        else "你是专业、友善的英语口语教练。一次性产出整份结构化报告。"
    )
    sections = [
        role_line,
        "",
        _for_subject(GROUNDING_RULES, subject),
        "",
        _for_subject(LANGUAGE_RULES, subject),
        "",
        mode_section,
        "",
        _for_subject(DIAGNOSTIC_INSTRUCTIONS, subject),
        "",
        f"—— {subject} transcript（逐字，证据只能引自这里）——",
        transcript_text,
        "",
        "—— 客观信号（确定性，作为判断输入 / 佐证）——",
        signals_json,
...
def _for_subject(text: str, subject: str) -> str:
    """共享 prompt 段以「考生」书写；非雅思语境整体替换称谓（如情景→「用户」）。"""
    return text if subject == "考生" else text.replace("考生", subject)
...
def _live_feedback_block(lf: LiveFeedback | None) -> str | None:
    ...
    if lf is None or (not lf.corrections and not lf.teachings):
        return None
    lines = [
        "—— 会话内即时反馈实录（系统日志，非用户 transcript：其中的英文句子"
