...
- [x] grammar_note 语法纠错 FC（对照式口头纠正 + 出现即报）——2026-06-07 PR #33：单 tool 双声明（与 language_help 同 `language_help_tool(case)` 生成）+ 应答台三态（回答最前对照纠正 "say X, not Y" 两变体轮换 / 中文求助同轮后置 / 静默仅剩 10s 防双发与 mixed_cn recast 重叠）+ `correction` 事件（spoken 标记，静默也发）；检出阈值经用户两轮决策定为出现即报、每轮至多一条、口语省略豁免（原 45s 间隔/同类上限两道闸移除）；288 测绿（隔离 worktree 验证）+ review PASS + 真冒烟（wants→want 对照实锤）；已知限制：「最前」是 prompt 级约束，流式下偶有先说半句再纠正
- [ ] case 选择页（验证 + review）
...
2026-06-07 — **方式 A live prompt 质量批次**（PR #31，4 维度 workflow 审阅 + 逐项对抗复核产出）：①开场连发修复——_OPENING_PROMPT 拆单动作（greet+自报姓名+问全名后 STOP 等答）、persona PART 1 两轮制、看门狗补发改幂等 nudge（防慢首包二次问候）；②指令防泄漏——方括号规则升 ABSOLUTE OUTPUT RULE 首条（禁读/禁转述/禁旁白）、persona 去全部 "the system" 元叙述、念题指令纯祈使化、切断/force 去解说断言、全文本锚碰撞审计零误触发；③随机考官音色——EXAMINER_VOICES 注册表（8 音色↔名字同源）每场随机抽、LIVE_VOICE 可固定、connect_live 增 voice 参数、情景不参与。pytest 275 绿（worktree 验证）+ review PASS / 无阻塞
2026-06-07 — **方式 B 对齐 live 考试规则**（PR #32，5 topic 调研 workflow + 交叉复核 + 用户拍板 D1-D5/A2/A3）：交叉复核点破关键 shape 差异——B 的 P2 原是 8 卡连录而非单卡长谈。落地：P2 每场随机抽 1 张卡 + p1/p3 抽 5（题库作随机池，方式 A cue card 路径零影响）；TTS 只读题面（bullets 不口播，p2 音频 8/8 重生成）；judge module_p2 加单卡长谈语义 + speaking_time_s 对照 1-2 分钟达标度、exam 分支加考试机制免责（负向 pin 钉死不进 B）。前端随轨代改：Record.jsx p2 读题后 60s 备题（I'm ready 提前）+ 满 2 分钟收尾提示、speechText 去 bullets（vitest 107 绿 + lint + build）。skip 项定性记录：开场核身/P3-P2 绑定/对话回路/TTS 音色随机化（成本不值）。pytest 289 绿 + review PASS / ⚠️ 本日多次与并行 backend session 同树作业：选择性暂存（剥 hunk/换内容暂存）+ 干净 worktree 验证 + 备份恢复三招保两轨零互踩 / 下次：P9 余 🔴 两项（pyaudio / _pick_cue_card）或 M3 报告参考分标注
2026-06-07 — **P5 grammar_note 完成**（PR #33）：语法纠错第三 FC 落地（检出归模型/呈现归代码三态；对照模板 {fixed} 先于 {original}）；阈值经用户实测反馈从「影响理解+存疑不报+45s+同类上限2」放宽为「出现即报+每轮一条+10s 防双发」——定位过程实锤一次旧进程坑（:8000 19:57 快照无 grammar 声明，判因时先查进程版本再怀疑模型）。前后端现以本 session 后台任务托管（APP_RELOAD=0）。无阻塞。下次：handoff/inbox/012 correction 卡片前端接入；候选：teaching/correction 落库进报告（学习闭环）
2026-06-07 — **报告语言规范 + 情景报告结构两连发**（PR #34 + PR #35）：①judge prompt 新增 LANGUAGE_RULES 段（所有模式注入）——录音引用一字不改 / rewrite 纯英文 / 建议中文嵌英文 / 解释字段一律中文 / 枚举不翻译，报告语言从隐式行为变显式契约；seed 7 条 pattern 中文化。②情景报告删 rewrites（grammar_note 会话内已即时纠正）+ 新增 diagnostics.summary 末尾总结（先肯定→点问题→给方向，一段简洁中文）；双向系统强制（情景清 rewrites / 雅思剥 summary），漏填降级 null + warning。295 测绿 + 双 review PASS + 用户真链路验收（API 级钉死：seed-02 rewrites=[] 且 summary 在位 / seed-07 summary=None）。⚠️ 本日并行撞车两次：提交落错分支（cherry-pick + branch -f 手术恢复，零丢失）/ handoff 012 撞号（改投 013）。前端件：013 标签中文化 + 014 总结段渲染已投 inbox。下次：PR-3 live FC 反馈进报告（correction/teaching 收集 → 确定性 live_feedback 区 + 喂 judge；仅情景，雅思不加 FC——用户拍板 2026-06-07）
2026-06-07 — **情景 live FC 反馈进报告**（PR #36，学习闭环候选项落地）：grammar_note/language_help tool 实录双路进报告——①Report.live_feedback 系统确定性直落（纠错 original→fixed+note+spoken / 求助 chinese→english+example，与 WS 事件同源同序、WS 死照收、零事件 null）②注入 judge prompt 作输入材料（块头钉死不得引进 evidence）；收口 tee.drain() 后从应答台导出经 finalize 透传，run_judge 单点模式守门（仅情景，雅思不加 FC——用户拍板）。随带修用户实测的称谓泄漏：共享 prompt 段「考生/考官」带歪情景输出用词→称谓随模式（情景换「用户」+ 开场白去考官 + 显式禁用）。307 测绿 + review PASS（W1 prompt 边界强化已修；W2 end_session 瞬间末批 tool_call 竞态记已知边际，损失上限一条）。前端件 015 已投（反馈区两小节 + null 防御）。⚠️ inbox 010-015 六件前端交接件待消化（013/014/015 为报告页三连）。下次：等前端消化交接件后整体联调报告页，或 P9 余 🔴 两项（pyaudio / _pick_cue_card）
