...
async def live_ws(websocket: WebSocket) -> None:
    ...
    try:
        async with connect_live() as live_session:
            ...
            # end_session → 自动触发课后 judge（SCHEMA §6.1）。回调在消费到
            # end_session 的瞬间调度成独立 task：前端发完可能立即断开并跳
            # /report/{session_id} 轮询，本协程随时被取消，不能事后再触发。
            await bridge(
                websocket,
                live_session,
                on_end_session=lambda: _schedule_finalize(session_id),
            )
...
def _schedule_finalize(session_id: str) -> None:
    """把 finalize（whisper 合并 + 一次 judge，CPU/网络阻塞活）丢线程后台跑。"""
